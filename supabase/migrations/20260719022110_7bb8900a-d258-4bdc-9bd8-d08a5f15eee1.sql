...
CREATE TABLE public.ranks (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  rank_number INT NOT NULL UNIQUE, name TEXT NOT NULL,
  min_xp INT NOT NULL, benefits JSONB NOT NULL DEFAULT '[]'::jsonb,
  icon TEXT, color TEXT,
  status public.entity_status NOT NULL DEFAULT 'active',
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
...
CREATE TABLE public.ams_tickets (
  ...
  subject text NOT NULL, description text, product text, category text,
  priority public.ams_priority NOT NULL DEFAULT 'medium',
  status public.ams_status NOT NULL DEFAULT 'draft',
  department text, team text, expected_resolution_at timestamptz,
