...
REVOKE EXECUTE ON FUNCTION public.can_post_in_chat(uuid, uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.is_chat_participant(uuid, uuid) TO service_role;
GRANT EXECUTE ON FUNCTION public.can_post_in_chat(uuid, uuid) TO service_role;
...
BEGIN
  ...
  RETURN NEW;
END; $$;
REVOKE EXECUTE ON FUNCTION public.bump_chat_conversation() FROM PUBLIC, anon, authenticated;
